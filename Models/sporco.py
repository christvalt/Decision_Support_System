...
for i in indicesName:
      ...
      datafrane.append(pd.read_csv(i , delimiter=',', decimal='.', names=['values'], header=0, error_bad_lines=False))
...
#f = pd.read_csv("../All_Bonds.csv" , delimiter=',', decimal='.', names=['values'], header=0, error_bad_lines=False)   
df = pd.read_csv("../%s.csv" % i, delimiter=',', decimal='.', names=['values'], header=0, error_bad_lines=False)

aValues = df[i].to_numpy() # array of values data
print("test"+aValues)
logdata = np.log(aValues) # log transform

cutpoint = int(len(datafrane[i]) * 0.92)
...
if len(sys.argv) == 2:
    ...
    for i in range(len(indices)):
        ...
        f, horizon_data_length = forcast(indices[i])
